...
if __name__ == "__main__":
    ...
    os.environ["CUDA_VISIBLE_DEVICES"] = "7,4,8"
    ...
    start_step = 0
    # torch.load("domainbed/results/Alg30729/8c979c97ad8c313480331f35d6e20b83/model.pkl")
    algorithm_dict = None
    ...
    hp_dict={}
    hp_dict['kdelta_lower_bound']=args.kdelta_lower_bound
    hp_dict['kdelta_upper_bound']=args.kdelta_upper_bound
    hp_dict['at_eps_lb']=args.at_eps_lb
    hp_dict['at_eps_ub']=args.at_eps_ub
    hp_dict['at_alpha_lb']=args.at_alpha_lb
    hp_dict['at_alpha_ub']=args.at_alpha_ub
    hp_dict['cb_rank_lb']=args.cb_rank_lb
    hp_dict['cb_rank_ub']=args.cb_rank_ub
    hp_dict['kat_alpha_step_lb']=args.kat_alpha_step_lb
    hp_dict['kat_alpha_step_ub']=args.kat_alpha_step_ub
    hp_dict['B_lr_lb']=args.B_lr_lb
    hp_dict['B_lr_ub']=args.B_lr_ub
    hp_dict['A_lr_lb']=args.A_lr_lb
    hp_dict['A_lr_ub']=args.A_lr_ub
    hp_dict['lr_lb']=args.lr_lb
    hp_dict['lr_ub']=args.lr_ub
    hp_dict['bsz_lb']=args.bsz_lb
    hp_dict['bsz_ub']=args.bsz_ub
    hp_dict['wd_lb']=args.wd_lb
    hp_dict['wd_ub']=args.wd_ub
    hp_dict['dpot']=args.dpot
    hp_dict['resnet18']=args.resnet18
    hp_dict['llr_lb']=args.llr_lb
    hp_dict['llr_ub']=args.llr_ub
    hp_dict['emlr_lb']=args.emlr_lb
    hp_dict['emlr_ub']=args.emlr_ub
    '''
    if args.hparams_seed == 0:
        hparams = hparams_registry.default_hparams(
            args.algorithm, args.dataset,hp_dict)
    else:
    '''   
    hparams = hparams_registry.random_hparams(args.algorithm, args.dataset,
                                                  misc.seed_hash(args.hparams_seed, args.trial_seed),hp_dict)
    hparams['KAT_num_iter']=args.KAT_num_iter
    hparams['CBAT_num_iter']=args.CBAT_num_iter
    hparams['CBAT2_num_iter']=args.CBAT2_num_iter    
    hparams['is_cmnist']=args.is_cmnist
    ...
    for step in range(start_step, n_steps):
        ...
        if args.task == "domain_adaptation":
            ...
            uda_device = None
        step_vals = algorithm.update(minibatches_device, uda_device,step,delta_dir=args.visualization_dir)
        checkpoint_vals['step_time'].append(time.time() - step_start_time)

        for key, val in step_vals.items():
            checkpoint_vals[key].append(val)
        if (step % checkpoint_freq == 0) or (step == n_steps-1):  # modified by ZC
            results = {
                'step': step,
