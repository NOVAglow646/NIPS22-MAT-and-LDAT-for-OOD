...
class SelectionMethod:
    ...
    def sweep_acc(self, records):
        ...
        if len(_hparams_accs):
            #_hparams_accs第一个维度i是不同超参数的测试，_hparams_accs[i][0]为
            # acc{'val_acc': 0.3909129875696528, 'test_acc': 0.398189318047892}
            #'val_acc'为该组超参数的最后一个epoch的env2_out_acc（验证集acc），'test_acc'为env2_out_acc，测试集正确率
            #第二个维度为所有epoch记录。_hparams_accs[i][0][0]为step=0，_hparams_accs[i][0][1]为step=50，_hparams_accs[i][0][2]为step=100，以此类推
            #print(len(_hparams_accs[0][1]))#
            print("Env=",(_hparams_accs[0][1][0]['args']['test_envs']))
            #print(len(_hparams_accs))#1
            print("Best result=",(_hparams_accs[0][0]))
            #print("Best result=",(_hparams_accs[0][1]))#所有epoch的超参数等信息
            print("Best result's model path=",(_hparams_accs[0][1][len(_hparams_accs[0][1])-1]['args']['output_dir']))
            print('\n')
